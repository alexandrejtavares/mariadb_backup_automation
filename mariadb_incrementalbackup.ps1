...
If (-Not (Test-Path -Path "$Env:BACKUP_PATH\full\*")) {
...
try{
    If (Test-Path -Path "$Env:BACKUP_PATH\incremental\"){
        ...
        New-Item -Path "$Env:BACKUP_PATH\incremental\" -ItemType Directory
        WriteLog("$Env:BACKUP_PATH\incremental\ folder created successfully")
    }
} catch {
